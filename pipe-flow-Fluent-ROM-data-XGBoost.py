...
# Fitting XGBoost model to the dataset

from sklearn import xgboost

regressor = xgboost.XGBRegressor(n_estimators = 100, random_state = 0)
...
plt.scatter(X_test[:, 0], y_pred, s=100, c = 'blue', label = 'XGBoost predictions')
plt.xlabel('Inlet Velocity')
plt.ylabel('Total Heat Transfer Rate at outlet')
plt.title('Comparison of XGBoost predictions with original CFD simulation data for the testset')
plt.legend()
plt.show()


plt.scatter(X_test[:, 1], y_test, s=100, c = 'red', label = 'CFD simulation data')
plt.scatter(X_test[:, 1], y_pred, s=100, c = 'blue', label = 'XGBoost predictions')
