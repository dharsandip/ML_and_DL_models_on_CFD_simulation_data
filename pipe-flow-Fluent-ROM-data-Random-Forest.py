...
plt.scatter(X_test[:, 0], y_pred, s=100, c = 'blue', label = 'Random Forest predictions')
plt.xlabel('Inlet Velocity')
plt.ylabel('Total Heat Transfer Rate at outlet')
plt.title('Comparison of Random Forest predictions with original CFD simulation data for the testset')
plt.legend()
plt.show()


plt.scatter(X_test[:, 1], y_test, s=100, c = 'red', label = 'CFD simulation data')
plt.scatter(X_test[:, 1], y_pred, s=100, c = 'blue', label = 'Random Forest predictions')
